    ...
    def __init__(self, file_name) -> None:
        ...
        with open(self.file_name) as file:
            for rout in file:
                ...
                next_hop_with_mtu = (next_hop, mtu)
                self.round_robin_logic[port_range]["routes"].append(next_hop_with_mtu)


    def check_routes(self, destination_address : tuple[str, int]) -> tuple[tuple[str, int], int] | None:
        """
        Retorna el par (dirección, MTU) de la ruta del siguiente salto para 
        llegar a la dirección de destino recibida. La dirección es una tupla 
        (IP, puerto). En caso de haber varias rutas disponibles para llegar a 
        un destino, se utiliza Round Robin para decidir cual ruta tomar.
        ...
        for port_range in self.port_ranges:
            if port_range[0] <= dest_port <= port_range[1]:
                ...
                next_hop_dir_mtu = self.round_robin_logic[port_range]["routes"][next_hop_index]
